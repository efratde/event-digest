"""
Scraper for Suzanne Dellal Center (מרכז סוזן דלל) — Israel's premier dance venue.
...
which contains:
  ...
  - .show-cats                     → category label (e.g. "מחול ישראלי", "ילדים", "הרצאות")
  - .single-event blocks           → one per performance date, each holds
                                     "<DD>/<MM>/<YYYY>", "<HH>:<MM>", hall name, price
  - free-text body w/ description, performers ("רקדנים/שותפים ליצירה:"), credits
  - JSON-LD WebPage entry with `datePublished` (used as tickets_opened_on proxy)

The center hosts mostly contemporary dance ("מחול"), but the user has explicitly
EXCLUDED ballet (בלט). Modern/contemporary dance ("מחול") is acceptable.
We filter out shows whose title or category mentions "בלט".
...
DURATION_RE = re.compile(r"(\d{2,3})\s*דק")
...
class SuzanneDellalScraper(Scraper):
    ...
    def fetch_shows(self) -> Iterable[Show]:
        ...
        for url in detail_urls:
            ...
            if show is None:
                continue
            # Ballet filter — father has explicitly excluded ballet.
    ...
    def _fetch_detail(self, url: str) -> Show | None:
        ...
        director = self._extract_field(soup, ["כוריאוגרפיה", "כוריאוגרף", "במאי", "בימוי"])
    ...
    def _extract_performances(soup) -> list[datetime]:
        ...
        # Fallback: header line "יום שני, 11 מאי, 2026, 20:30" — only if no .single-event found.
        if not performances:
    ...
    def _extract_performers(soup) -> list[str]:
        for label in ["רקדנים/שותפים ליצירה", "רקדנים", "מבצעים", "משתתפים", "בכיכובם"]:
            for el in soup.find_all(string=re.compile(rf"{re.escape(label)}\s*[:：]")):
                parent = el.parent
                txt = parent.get_text(" ", strip=True)
                m = re.search(rf"{re.escape(label)}\s*[:：]\s*(.+)", txt)
    ...
    def _extract_duration(soup) -> int | None:
        # Look in .show-details first to avoid matching unrelated "דק" elsewhere.

...
zappa-club.co.il listing as the indoor Zappa venues. The HTML structure is
identical to ``zappa_tlv.py`` — one ``article.listing-item`` per night,
``time[datetime]`` for the ISO performance datetime, ``data-event-id`` for
...
Note: this is a summer-only venue. Off-season (roughly October-March) the
listing may be empty or contain only a handful of advance announcements;
...
class ShuniScraper(Scraper):
    ...
    def fetch_shows(self) -> Iterable[Show]:
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            try:
                ...
                self.log.warning("Shuni page %d failed: %s", page_num, e)
                break

            soup = BeautifulSoup(r.text, "lxml")
            arts = soup.select("article.listing-item")
            self.log.info("Shuni page %d: %d cards", page_num, len(arts))
            if not arts:
                break
    ...
    def _collect_card(self, art, groups: dict[str, dict]) -> None:
        ...
        # Shuni-only filter — the URL slug always carries the venue name when
        # present. Belt-and-suspenders against the listing accidentally
        # cross-promoting another Zappa branch.
        if href and not self._is_shuni(href):
            return
    ...
    def _parse_dt(raw: str) -> datetime | None:
        """Parse '2026-05-26T20:45:00.000+03:00' (with or without ms / colon)."""
        if not raw:
            return None
        s = raw.strip()
        s = re.sub(r"\.\d+", "", s)
